...
CREATE TABLE IF NOT EXISTS `admaptec_jmln2`.`cmb_pago`(
  ...
  `ULTIMOMES` INT NOT NULL,
  `ANIO` INT,
  PRIMARY KEY( `IDCURSO`,  `IDCATEDRATICO`,  `IDALUMNO`, `NUMRECIBO`))
ENGINE = InnoDB
DEFAULT CHARACTER SET = latin1
COLLATE = latin1_spanish_ci;

DROP TABLE IF EXISTS cmb_recibo;
CREATE TABLE IF NOT EXISTS `admaptec_jmln2`.`cmb_recibo`(
   `NUMRECIBO` INT NOT NULL,
   `IDALUMNO` INT NOT NULL,
   `FECHA` DATE NOT NULL,
